...
- **現在の6セクション構成**:
  ...
  2. **VALUE（価値観）**: 昨日の自分を超え続ける
     - 日本語サブタイトル追加
  
  3. **CREDO（大事にしている行動指針）**: 4つの行動指針（2カラムレイアウト）
     - 日本語サブタイトル追加
     - **No.1にこだわれ**
  ...
  5. **ENVIRONMENT（働く環境）**: 統合セクション
     - **数字で見るSenjin**:
       ...
       - 男女比率: 7:3のSVGドーナツチャート
       - 平均年齢: SVGドーナツチャート（25.6歳、20代90%強調）
       - 拠点数: 画像埋め込み（locations-map.png）4拠点（東京、岩国、天草、福岡）
     - **福利厚生**:
       - 寮制度: オフィス横のタワーマンション
       - 家賃補助制度: 徒歩圏内5万円/月
...
- **デザイン特徴**:
  - ダークテーマ継続（黒背景）
  - 大型タイトル（全セクション大文字表記に統一）
  - SVGグラフによる視覚的な情報提示
  - 固定CTAボタン：
    - 常時展開表示（ホバー不要）
    - エントリーボタン：水色
    - お問い合わせボタン：ピンク色で差別化
  - スクロール時ヘッダー：黒背景固定（白にならないよう修正済み）
